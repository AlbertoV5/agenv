...
async function transpileFile(srcPath: string, outPath: string) {
  ...
  // Skip .yaml and .json files as they are imported with { type: "text" }
  jsCode = jsCode.replace(/from\s+["'](\.\.?\/[^"']+?)["']/g, (match, path) => {
    if (!path.endsWith('.js') && !path.endsWith('.json') && !path.endsWith('.yaml') && !path.includes('.js#')) {
      return `from "${path}.js"`
    }
    return match
  })
...
try {
  ...
  // Step 2: Copy default config files (yaml, json) to dist
  ...
  for await (const file of walk("src")) {
    ...
    if (ext === ".yaml" || ext === ".json") {
      const outPath = file.replace(/^src/, "dist/src")
      await mkdir(dirname(outPath), { recursive: true })
  ...
  console.log(`✅ Copied ${defaultFileCount} default config files`)

  // Step 3: Generate declaration files
