...
// src/lib/tasks-md.ts
var THREAD_HEADER_REGEX = /^Thread\s+(\d+):\s*([^@]*?)(?:\s+@agent:([a-zA-Z0-9_-]*))?\s*$/i;
...
function main(argv = process.argv) {
  ...
  try {
    ...
    const workDir = getWorkDir(repoRoot);
    const streamDir = join5(workDir, stream.id);
    const tasksMdPath = join5(streamDir, "TASKS.md");
    const tasksJsonPath = join5(streamDir, "tasks.json");
