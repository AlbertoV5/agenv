...
import { existsSync as existsSync6, readFileSync as readFileSync5, unlinkSync } from "fs";
...
function checkTasksApprovalReady(repoRoot, streamId) {
  const { readFileSync: readFileSync4, existsSync: existsSync5 } = __require("fs");
  ...
  const tasksMdPath = join4(workDir, streamId, "TASKS.md");
  if (!existsSync5(tasksMdPath)) {
    return {
      ready: false,
  ...
  const content = readFileSync4(tasksMdPath, "utf-8");
...
// src/lib/prompts.ts
init_repo();
import { existsSync as existsSync5, readFileSync as readFileSync4, mkdirSync, writeFileSync as writeFileSync2 } from "fs";
...
function getPromptContext(repoRoot, streamId, threadIdStr) {
  ...
  const workDir = getWorkDir(repoRoot);
  const planPath = join4(workDir, streamId, "PLAN.md");
  if (!existsSync5(planPath)) {
    throw new Error(`PLAN.md not found at ${planPath}`);
  }
  const planContent = readFileSync4(planPath, "utf-8");
  const errors = [];
...
function getPromptRelativePath(context) {
  ...
  return join4(context.streamId, "prompts", `${stagePrefix}-${safeStageName}`, `${context.batch.prefix}-${safeBatchName}`, `${safeThreadName}.md`);
}
function savePromptToFile(repoRoot, context, content) {
  const workDir = getWorkDir(repoRoot);
  const relPath = getPromptRelativePath(context);
  const fullPath = join4(workDir, relPath);
  try {
...
function serializeTasksMdToJson(repoRoot, streamId) {
  try {
    ...
    const tasksJsonPath = join5(workDir, streamId, "tasks.json");
