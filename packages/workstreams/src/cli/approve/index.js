...
function getPlanMdPath(repoRoot, streamId) {
  const workDir = getWorkDir(repoRoot);
  return join4(workDir, streamId, "PLAN.md");
}
...
function checkTasksApprovalReady(repoRoot, streamId) {
  const { readFileSync: readFileSync5, existsSync: existsSync6 } = __require("fs");
  ...
  const tasksMdPath = join5(workDir, streamId, "TASKS.md");
  if (!existsSync6(tasksMdPath)) {
    return {
      ready: false,
  ...
  const content = readFileSync5(tasksMdPath, "utf-8");
...
function getGitHubConfigPath(repoRoot) {
  return join5(repoRoot, "work", "github.json");
}
...
function getWorkstreamGitHubPath(repoRoot, streamId) {
  return join6(getWorkDir(repoRoot), streamId, "github.json");
}
...
function createStageApprovalCommit(repoRoot, stream, stageNum, stageName) {
  try {
    ...
    execSync2(`git commit -m "${title}" -m "${escapedBody}"`, {
      cwd: repoRoot,
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    });
...
function generateTasksMdAfterApproval(repoRoot, streamId, streamName) {
  try {
    const workDir = getWorkDir(repoRoot);
    const streamDir = join7(workDir, streamId);
    const tasksMdPath = join7(streamDir, "TASKS.md");
    const planMdPath = join7(streamDir, "PLAN.md");
    ...
    const overwritten = existsSync6(tasksMdPath);
    const planContent = readFileSync5(planMdPath, "utf-8");
    const errors = [];
...
import { existsSync as existsSync8, readFileSync as readFileSync7, unlinkSync } from "fs";
...
// src/lib/prompts.ts
init_repo();
import { existsSync as existsSync7, readFileSync as readFileSync6, mkdirSync, writeFileSync as writeFileSync2 } from "fs";
import { join as join8, dirname as dirname4 } from "path";
init_tasks();
function parseThreadId(threadIdStr) {
...
function savePromptToFile(repoRoot, context, content) {
  ...
  try {
    mkdirSync(dirname4(fullPath), { recursive: true });
...
function serializeTasksMdToJson(repoRoot, streamId) {
  try {
    const workDir = getWorkDir(repoRoot);
    const tasksMdPath = join9(workDir, streamId, "TASKS.md");
    const tasksJsonPath = join9(workDir, streamId, "tasks.json");
    if (!existsSync8(tasksMdPath)) {
      return {
        success: false,
        taskCount: 0,
    ...
    const content = readFileSync7(tasksMdPath, "utf-8");
...
function handleRevisionApproval(repoRoot, stream, cliArgs) {
  const workDir = getWorkDir(repoRoot);
  const streamDir = join10(workDir, stream.id);
  const planMdPath = join10(streamDir, "PLAN.md");
  const tasksMdPath = join10(streamDir, "TASKS.md");
  if (!existsSync9(planMdPath)) {
    console.error(`Error: PLAN.md not found at ${planMdPath}`);
    process.exit(1);
  }
  const planContent = readFileSync8(planMdPath, "utf-8");
  const errors = [];
  const doc = parseStreamDocument(planContent, errors);
  if (!doc) {
