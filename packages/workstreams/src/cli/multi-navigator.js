...
async function main(argv = process.argv) {
  ...
  const { loadIndex: loadIndex2, getResolvedStream: getResolvedStream2 } = await Promise.resolve().then(() => (init_lib(), exports_lib));
  const idx = loadIndex2(repoRoot);
  const stream = getResolvedStream2(idx, streamId);
  const planPath = join5(repoRoot, "agenv", "work", stream.id, "PLAN.md");
  const { getWorkDir: getWorkDir2 } = await Promise.resolve().then(() => (init_repo(), exports_repo));
  const wDir = getWorkDir2(repoRoot);
  const pPath = join5(wDir, stream.id, "PLAN.md");
  const content = readFileSync4(pPath, "utf-8");
