...
import { workstreamBranchExists, formatBranchName, getWorkstreamBranchName } from "../lib/github/branches.ts"
...
function parseCliArgs(argv: string[]): CompleteStreamCliArgs | null {
  ...
  const parsed: CompleteStreamCliArgs = { commit: true, pr: true, draft: false, json: false }
...
function performGitOperations(
  ...
  try {
    ...
    if (!statusOutput) {
      ...
      execSync(`git commit -m "${commitMessage}" -m "${commitBody.replace(/"/g, '\\"')}"`, {
        cwd: repoRoot,
        encoding: "utf-8",
        stdio: ["pipe", "pipe", "pipe"],
      })
 ...
 */
function checkForUnpushedCommits(repoRoot: string, branchName: string): boolean {
  try {
 ...
 */
function getTaskSummary(repoRoot: string, streamId: string): {
  total: number
  completed: number
  inProgress: number
  pending: number
 ...
 */
function storeCompletionMetadata(
  repoRoot: string,
  streamId: string
): void {
...
): Promise<{ errors: string[]; warnings: string[]; branchName?: string; targetBranch: string }> {
  const errors: string[] = []
