...
function generateStageReport(repoRoot, streamId, stageRef) {
  ...
  const workDir = getWorkDir(repoRoot);
  const planPath = join4(workDir, stream.id, "PLAN.md");
...
function generateReportTemplate(repoRoot, streamId) {
  ...
  const workDir = getWorkDir(repoRoot);
  const planPath = join5(workDir, stream.id, "PLAN.md");
...
function parseReport(repoRoot, streamId) {
  ...
  const workDir = getWorkDir(repoRoot);
  const reportPath = join5(workDir, stream.id, "REPORT.md");
...
function handleInit(repoRoot, streamId) {
  ...
  const workDir = getWorkDir(repoRoot);
  const reportPath = join6(workDir, stream.id, "REPORT.md");
  ...
  if (cliArgs.stage) {
    ...
    try {
      ...
      if (cliArgs.json) {
        ...
        if (cliArgs.output) {
          writeFileSync3(cliArgs.output, output3);
      ...
      if (cliArgs.output) {
        writeFileSync3(cliArgs.output, output2);
        console.log(`Stage report written to ${cliArgs.output}`);
      } else {
        console.log(output2);
      }
...
`);
  ...
  if (cliArgs.output) {
    writeFileSync3(cliArgs.output, output);
    console.log(`Report written to ${cliArgs.output}`);
  } else {
    console.log(output);
  }
