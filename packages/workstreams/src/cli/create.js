...
function generateStream(args) {
  ...
  const streamDir = join2(workDir, streamId);
  mkdirSync(streamDir, { recursive: true });
  const docsDir = join2(streamDir, "docs");
  mkdirSync(docsDir, { recursive: true });
  const planContent = generatePlanMd(streamId, args.name, args.stages ?? 1);
  atomicWriteFile(join2(streamDir, "PLAN.md"), planContent);
  atomicWriteFile(join2(streamDir, "tasks.json"), JSON.stringify(generateTasksJson(streamId), null, 2));
