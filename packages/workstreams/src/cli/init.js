...
function getGitHubConfigPath(repoRoot) {
  return join3(repoRoot, "work", "github.json");
}
...
function getNotificationsConfigPath(repoRoot) {
  return join4(repoRoot, "work", "notifications.json");
}
...
function getSynthesisConfigPath(repoRoot) {
  return join5(repoRoot, "work", "synthesis.json");
}
...
async function main(argv) {
  ...
  try {
    ...
    if (!existsSync3(githubConfigPath) || force) {
      console.log(`${force && existsSync3(githubConfigPath) ? "Overwriting" : "Creating"} github.json (disabled by default)...`);
      await saveGitHubConfig(repoRoot, DEFAULT_GITHUB_CONFIG);
    } else {
      console.log("github.json already exists, skipping.");
    }
    const indexPath = getIndexPath(repoRoot);
    if (!existsSync3(indexPath) || force) {
      console.log(`${force && existsSync3(indexPath) ? "Overwriting" : "Initializing"} index.json...`);
    ...
    if (!existsSync3(agentsPath) || force) {
      console.log(`${force && existsSync3(agentsPath) ? "Overwriting" : "Initializing"} agents.yaml...`);
      writeFileSync2(agentsPath, DEFAULT_AGENTS_YAML, "utf-8");
    } else {
      console.log("agents.yaml already exists, skipping.");
    }
    const notificationsPath = getNotificationsConfigPath(repoRoot);
    if (!existsSync3(notificationsPath) || force) {
      console.log(`${force && existsSync3(notificationsPath) ? "Overwriting" : "Initializing"} notifications.json...`);
      const defaultConfig = getDefaultNotificationsConfig();
      writeFileSync2(notificationsPath, JSON.stringify(defaultConfig, null, 2), "utf-8");
    } else {
      console.log("notifications.json already exists, skipping.");
    }
    const synthesisPath = getSynthesisConfigPath(repoRoot);
    if (!existsSync3(synthesisPath) || force) {
      console.log(`${force && existsSync3(synthesisPath) ? "Overwriting" : "Initializing"} synthesis.json...`);
      ...
      writeFileSync2(synthesisPath, JSON.stringify(defaultSynthesisConfig, null, 2), "utf-8");
    } else {
