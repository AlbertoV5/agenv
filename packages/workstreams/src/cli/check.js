...
function consolidateStream(repoRoot, streamId, _dryRun = false) {
  ...
  const content = readFileSync2(planMdPath, "utf-8");
...
function findMissingInputFiles(repoRoot, planMdPath, files) {
  const planDir = dirname2(planMdPath);
  const missing = [];
  for (const file of files) {
    ...
    const exists = candidates.some((p) => existsSync4(p));
    if (!exists) {
      missing.push(file);
    }
  }
...
Run with --help for usage information.`);
  ...
  if (cliArgs.subcommand === "plan") {
    ...
    const content = readFileSync3(planMdPath, "utf-8");
