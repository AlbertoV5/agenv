...
function getAgentsYamlPath(repoRoot) {
  return join2(getWorkDir(repoRoot), "agents.yaml");
}
...
var CONFIG_PATH = join5(homedir(), ".config", "agenv", "notifications.json");
...
function getNotificationsConfigPath(repoRoot) {
  return join6(repoRoot, "work", "notifications.json");
}
...
class TerminalNotifierProvider {
  ...
  isAvailable() {
    ...
    try {
      execSync2("which terminal-notifier", { stdio: "ignore" });
...
function getSynthesisConfigPath(repoRoot) {
  return join7(repoRoot, "work", "synthesis.json");
}
...
import { readFileSync as readFileSync8, appendFileSync, existsSync as existsSync10 } from "node:fs";
...
function getPromptFilePathFromMetadata(repoRoot, streamId, stageNum, stageName, batchNum, batchName, threadName) {
  ...
  return join8(workDir, streamId, "prompts", `${stagePrefix}-${safeStageName}`, `${batchPrefix}-${safeBatchName}`, `${safeThreadName}.md`);
}
...
function validateThreadPrompts(threads) {
  ...
  for (const thread of threads) {
    if (!existsSync11(thread.promptPath)) {
...
Capturing opencode session IDs and synthesis output...`);
    for (const mapping of threadSessionMap) {
      ...
      if (existsSync13(sessionFilePath)) {
        try {
          const opencodeSessionId = readFileSync10(sessionFilePath, "utf-8").trim();
      ...
      if (existsSync13(workingAgentSessionPath)) {
        try {
          const workingAgentSessionId = readFileSync10(workingAgentSessionPath, "utf-8").trim();
      ...
      if (existsSync13(synthesisJsonPath)) {
        try {
...
function resolvePromptPathFromMetadata(repoRoot, streamId, metadata) {
  ...
  return join9(workDir, streamId, "prompts", `${stagePrefix}-${safeStageName}`, `${batchPrefix}-${safeBatchName}`, `${safeThreadName}.md`);
}
function resolvePromptPath(repoRoot, streamId, threadId) {
  const workDir = getWorkDir(repoRoot);
  const planPath = join9(workDir, streamId, "PLAN.md");
  if (!existsSync14(planPath)) {
    return null;
  }
  const planContent = readFileSync11(planPath, "utf-8");
  const errors2 = [];
...
// src/lib/fix.ts
import { readFileSync as readFileSync12, writeFileSync as writeFileSync3 } from "fs";
...
function getStreamPlanMdPath(repoRoot, streamId) {
  const workDir = getWorkDir(repoRoot);
  return join10(workDir, streamId, "PLAN.md");
}
...
async function executeRetry(repoRoot, streamId, threadStatus, agentName, models, dryRun, noTmux) {
  ...
  if (!promptPath || !existsSync15(promptPath)) {
