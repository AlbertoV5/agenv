...
function savePromptToFile(repoRoot, context, content) {
  ...
  const relPath = join5(context.streamId, "prompts", `${stagePrefix}-${safeStageName}`, `${context.batch.prefix}-${safeBatchName}`, `${safeThreadName}.md`);
  const fullPath = join5(workDir, relPath);
  try {
...
Run with --help for usage information.`);
  ...
  if (!cliArgs.threadId) {
    ...
    const { getWorkDir: getWorkDir2 } = await Promise.resolve().then(() => (init_repo(), exports_repo));
    ...
    const planPath = join6(workDir, stream.id, "PLAN.md");
    ...
    const errors = [];
    const doc = parseStreamDocument2(planContent, errors);
    if (!doc) {
