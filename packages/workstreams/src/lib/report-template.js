...
function generateReportTemplate(repoRoot, streamId) {
  ...
  const planPath = join2(workDir, stream.id, "PLAN.md");
  const planContent = readFileSync2(planPath, "utf-8");
  const errors = [];
...
function parseReport(repoRoot, streamId) {
  ...
  const workDir = getWorkDir(repoRoot);
  const reportPath = join2(workDir, stream.id, "REPORT.md");
  const content = readFileSync2(reportPath, "utf-8");
  const lexer = new x;
  const tokens = lexer.lex(content);
  const report = {
    streamId: stream.id,
