...
async function startThreadSessionLocked(repoRoot, streamId, threadId, agentName, model, sessionId) {
  const filePath = getThreadsFilePath(repoRoot, streamId);
  return withThreadsLock(filePath, () => {
    return startThreadSession(repoRoot, streamId, threadId, agentName, model, sessionId);
  });
}
async function completeThreadSessionLocked(repoRoot, streamId, threadId, sessionId, status, exitCode) {
  const filePath = getThreadsFilePath(repoRoot, streamId);
  return withThreadsLock(filePath, () => {
...
var VALID_SESSION_STATUSES = ["running", "completed", "failed", "interrupted"];
function validateSessionRecord(session, taskId) {
  const errors = [];
  ...
  } else if (isNaN(Date.parse(s.startedAt))) {
    errors.push({
      taskId,
      field: "startedAt",
  ...
  if (typeof s.status !== "string" || !VALID_SESSION_STATUSES.includes(s.status)) {
    errors.push({
      taskId,
      field: "status",
...
function validateTaskSessions(task) {
  const errors = [];
    ...
    if (!Array.isArray(task.sessions)) {
      ...
      if (task.currentSessionId !== undefined) {
        if (typeof task.currentSessionId !== "string") {
          ...
          const sessionExists = task.sessions.some((s) => s.sessionId === task.currentSessionId);
...
function validateTasksFileSessions(tasksFile) {
  const errors = [];
  for (const task of tasksFile.tasks) {
...
function getCurrentTaskSession(repoRoot, streamId, taskId) {
  const threadId = extractThreadIdFromTaskId2(taskId);
  const thread = getThreadMetadata(repoRoot, streamId, threadId);
  if (!thread || !thread.currentSessionId)
    return null;
  return thread.sessions.find((s) => s.sessionId === thread.currentSessionId) || null;
}
function getTaskSessions(repoRoot, streamId, taskId) {
  const threadId = extractThreadIdFromTaskId2(taskId);
  const thread = getThreadMetadata(repoRoot, streamId, threadId);
  if (!thread)
    return [];
...
async function startTaskSessionLocked(repoRoot, streamId, taskId, agentName, model, sessionId) {
  const filePath = getTasksFilePath(repoRoot, streamId);
...
async function completeTaskSessionLocked(repoRoot, streamId, taskId, sessionId, status, exitCode) {
  const threadId = extractThreadIdFromTaskId2(taskId);
...
function addTasks(repoRoot, streamId, newTasks) {
  ...
  tasksFile.tasks = Array.from(existingTasksMap.values()).sort((a, b) => a.id.localeCompare(b.id, undefined, { numeric: true }));
  writeTasksFile(repoRoot, streamId, tasksFile);
  return tasksFile;
}
...
function setTaskGitHubMeta(_repoRoot, _streamId, _taskId, _meta) {
  return null;
}
...
function groupTasksByStageAndBatchAndThread(tasks) {
  return groupTasks(tasks, { byBatch: true });
}
export {
  writeTasksFile,
  validateTasksFileSessions,
  validateTaskSessions,
  ...
  parseThreadId,
  parseTaskId,
  migrateSessionsToThreads,
  hasSessionsInTasksJson,
