...
function deleteThreadMetadata(repoRoot, streamId, threadId) {
  const threadsFile = loadThreads(repoRoot, streamId);
  if (!threadsFile)
    return false;
  const threadIndex = threadsFile.threads.findIndex((t) => t.threadId === threadId);
...
function getAllThreadMetadata(repoRoot, streamId) {
  const threadsFile = loadThreads(repoRoot, streamId);
  if (!threadsFile)
    return [];
...
async function modifyThreads(repoRoot, streamId, fn) {
  const filePath = getThreadsFilePath(repoRoot, streamId);
  return withThreadsLock(filePath, () => {
    let threadsFile = loadThreads(repoRoot, streamId);
    if (!threadsFile) {
...
function setThreadGitHubIssue(_repoRoot, _streamId, _threadId, _issue) {
  return null;
}
function getThreadGitHubIssue(_repoRoot, _streamId, _threadId) {
  return null;
}
...
function getSynthesisOutput(repoRoot, streamId, threadId) {
...
function setWorkingAgentSessionId(repoRoot, streamId, threadId, sessionId) {
...
function validateMigration(repoRoot, streamId, tasksFile) {
  const issues = [];
  const threadsFile = loadThreads(repoRoot, streamId);
  if (!threadsFile) {
