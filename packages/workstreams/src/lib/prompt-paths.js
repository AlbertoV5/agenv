...
function resolvePromptPathFromMetadata(repoRoot, streamId, metadata) {
  ...
  return join2(workDir, streamId, "prompts", `${stagePrefix}-${safeStageName}`, `${batchPrefix}-${safeBatchName}`, `${safeThreadName}.md`);
}
function resolvePromptPath(repoRoot, streamId, threadId) {
  const workDir = getWorkDir(repoRoot);
  const planPath = join2(workDir, streamId, "PLAN.md");
