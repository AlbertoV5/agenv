#!/bin/bash
# Release a minor version of @agenv/workstreams
# Usage: ./scripts/release-workstreams-minor.sh

set -e

cd "$(dirname "$0")/../packages/workstreams"

echo "📦 Releasing @agenv/workstreams minor version..."

# Bump version and create git tag
VERSION=$(bun pm version minor --no-git-tag-version | tail -1)
VERSION=${VERSION#v}
